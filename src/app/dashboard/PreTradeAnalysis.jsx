...
export default function PreTradeAnalysis() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.fundamental_analysis)
        form.append("fundamental_analysis", formData.fundamental_analysis);
      if (formData.additional_notes)
        form.append("additional_notes", formData.additional_notes);
      if (formData.strategy_id)
        form.append("strategy_id", parseInt(formData.strategy_id));
      if (formData.strategy_name)
        form.append("strategy_name", formData.strategy_name);
      formData.images.forEach((file) => form.append("images", file));

      const endpoint = editingAnalysis
        ? `/api/analyses/${editingAnalysis.id}`
        : "/api/analyses";
      const method = editingAnalysis ? "PUT" : "POST";
      const result = await apiCall(endpoint, { method, body: form });
      if (result) {
        const msg = editingAnalysis
  ...
  const normalizeForView = (analysis) => {
    ...
    return {
      ...
      confidence_level:
        analysis.confidence_level ?? analysis.confidencelevel ?? "-",
      timeframe: analysis.timeframe ?? "-",
      technical_analysis:
        analysis.technical_analysis ?? analysis.technicalanalysis ?? "",
      fundamental_analysis:
        analysis.fundamental_analysis ?? analysis.fundamentalanalysis ?? "",
      additional_notes:
        analysis.additional_notes ?? analysis.additionalnotes ?? "",
  ...
  const confirmDeleteAnalysis = async () => {
    ...
    try {
      setDeleting(true);
      await apiCall(`/api/analyses/${analysisToDelete.id}`, {
        method: "DELETE",
      });
    ...
    c === "High"
      ...
      : c === "Medium"
        ...
        : c === "Low"
          ? "error"
          : "default";
  const getTradeTypeColor = (t) =>
    (t || "Long") === "Long" ? "success" : "error";
  const formatDate = (s) => {
    try {
    ...
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          {!showForm && (
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[#3b82f6] text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-sm sm:text-base font-semibold hover:bg-[#2563eb] transition-all duration-200 shadow-lg"
            >
              <svg
      ...
      {success && (
        <div className="bg-green-50 border border-green-200 rounded-lg sm:rounded-xl p-3 sm:p-4">
          <div className="flex items-start">
            <div className="flex-1 text-green-600 text-sm sm:text-base">
              {success}
            </div>
            <button
              onClick={() => setSuccess("")}
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-lg sm:rounded-xl p-3 sm:p-4">
          <div className="flex items-start">
            <div className="flex-1 text-red-600 text-sm sm:text-base pr-2">
              {error}
            </div>
            <button
              onClick={() => setError("")}
          ...
          <div className="w-full max-w-4xl bg-white rounded-lg shadow-2xl h-[90vh] flex flex-col overflow-hidden my-4">
            <div className="bg-slate-100 px-4 sm:px-6 py-4 flex justify-between items-center border-b border-slate-200 flex-shrink-0">
              <h2 className="text-lg font-bold text-slate-900">
                {editingAnalysis
                  ? "Edit Pre-Trade Analysis"
                  : "New Pre-Trade Analysis"}
              </h2>
              <button
                type="button"
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-lg border border-white/20">
          <h3 className="text-base sm:text-lg font-semibold text-black mb-2">
            No Pre-Trade Analyses
          </h3>
          <p className="text-sm sm:text-base text-slate-600 mb-6">
            Start analyzing potential trades before executing them.
          </p>
          <button
            onClick={openNewAnalysis}
                      ...
                      <TableRow key={a.id} className="hover:bg-slate-50">
                        ...
                        <TableCell>
                          <Chip
                            label={a.trade_type}
                            size="small"
                            color={getTradeTypeColor(a.trade_type)}
                            sx={{ fontSize: "0.75rem" }}
                          />
                        </TableCell>
                        ...
                        <TableCell>
                          <Chip
                            label={a.confidence_level}
                            size="small"
                            color={getConfidenceColor(a.confidence_level)}
                            sx={{ fontSize: "0.75rem" }}
                          />
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell sx={{ textAlign: "center" }}>
                          ...
                          >
                            ...
                            <Tooltip title="Delete">
                              <IconButton
                                size="small"
                                onClick={() => requestDeleteAnalysis(a)}
                                sx={{ color: "#dc2626" }}
                              >
          ...
          <div className="md:hidden space-y-3">
            {analyses.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className="bg-white/90 backdrop-blur-sm rounded-xl p-4 shadow-lg border border-white/30 hover:shadow-xl transition-all"
                >
        ...
        <div className="fixed inset-0 z-[10001] bg-black/50 flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-white rounded-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="bg-slate-100 px-6 py-4 flex justify-between items-center border-b border-slate-200 sticky top-0">
              <h2 className="text-lg font-bold text-slate-900">
                Analysis - {selectedAnalysis.symbol}
              </h2>
              <button
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center gap-2 text-xs text-slate-600">
                ...
                <span className="font-medium">
                  {selectedAnalysis.trade_type}
                </span>
                <span className="ml-3">Confidence:</span>
                <span className="font-medium">
                  {selectedAnalysis.confidence_level}
                </span>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                  {[
                    {
                      label: "Current",
                      value: selectedAnalysis.current_price,
                      bg: "bg-slate-100",
                    },
                    {
                      label: "Entry",
                      value: selectedAnalysis.entry_price,
                      bg: "bg-blue-50",
                    },
                    {
                      label: "Target",
                      value: selectedAnalysis.target_price,
                      bg: "bg-green-50",
                    },
                    {
                      label: "Stop",
                      value: selectedAnalysis.stop_loss,
                      bg: "bg-red-50",
                    },
                    {
                      label: "Qty",
                      value: selectedAnalysis.quantity,
                      bg: "bg-slate-100",
                    },
                  ].map((item, idx) => (
                    <div
              ...
              {selectedAnalysis.images?.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-slate-700 mb-3">
                    Images ({selectedAnalysis.images.length})
                  </h4>
