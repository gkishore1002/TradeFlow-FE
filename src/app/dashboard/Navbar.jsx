...
const Navbar = ({
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/notifications?per_page=5&sort_order=desc`,
        {
          headers: {
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (notification.link) {
      // Robust mapping of links to dashboard tabs
      const link = notification.link.toLowerCase();

      // Mapping common aliases to internal tab IDs
      const tabMapping = {
        dashboard: "dashboard",
        journal: "journal",
        ...
        analysis: "analysis",
        logs: "logs",
        "trade-logs": "logs", // Map trade-logs to logs tab
        "success-logs": "logs",
        trade: "logs",
        charts: "charts",
      ...
      // Try to find a match in the link string
      let matchedTab = null;
      for (const [alias, tabId] of Object.entries(tabMapping)) {
        if (link.includes(alias)) {
          matchedTab = tabId;
          break;
        }
      }

      if (matchedTab && setActiveTab) {
        console.log(
          `🎯 Mapping notification link "${notification.link}" to tab "${matchedTab}"`,
        );
        setActiveTab(matchedTab);

        // Ensure we are on the dashboard page
        if (window.location.pathname !== "/dashboard") {
          router.push(`/dashboard?tab=${matchedTab}`);
        }
      } else {
        // Fallback to direct routing if no tab match or setActiveTab missing
  ...
  const getNotificationIcon = (type) => {
    ...
    return (
      <div
        className={`w-9 h-9 rounded-lg ${config.bgColor} flex items-center justify-center flex-shrink-0 ${config.iconColor}`}
      >
        {config.icon}
      </div>
    );
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-3 md:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 ml-1 sm:ml-2">
              <div
                className={`w-2 h-2 rounded-full animate-pulse ${backendConnected ? "bg-green-500" : "bg-orange-500"}`}
              ></div>
              <span
                className={`text-xs font-semibold whitespace-nowrap ${backendConnected ? "text-green-600" : "text-orange-600"}`}
              >
              ...
              {profileMenuOpen && (
                <div className="absolute right-3 sm:right-4 md:right-6 lg:right-8 top-full mt-2 w-56 sm:w-64 bg-white rounded-lg shadow-xl border border-slate-200 overflow-hidden z-[9999] animate-fade-in">
                  ...
                  <div className="px-4 sm:px-5 py-2 sm:py-3 bg-slate-50 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${backendConnected ? "bg-green-500" : "bg-orange-500"}`}
                      ></div>
                      <span
                        className={`text-xs font-medium ${backendConnected ? "text-green-600" : "text-orange-600"}`}
                      >
