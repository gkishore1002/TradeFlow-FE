...
export default function SuccessLogs() {
  ...
  const fetchStrategyWiseTrades = useCallback(
    async (page = 1, search = "") => {
      try {
        ...
        if (data?.items) {
          ...
          data.items.forEach((trade) => {
            ...
            if (pnl > 0) {
              ...
              grouped[strategyName].best_trade = Math.max(
                grouped[strategyName].best_trade,
                pnl,
              );
            } else if (pnl < 0) {
          ...
          Object.values(grouped).forEach((strategy) => {
            strategy.win_rate =
              strategy.total_trades > 0
                ? (
                    (strategy.success_trades / strategy.total_trades) *
                    100
                  ).toFixed(1)
                : 0;
            strategy.avg_pnl =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataObj.append(
        "entry_price",
        String(parseFloat(formData.entry_price)),
      );
      ...
      if (formData.trading_strategy) {
        ...
        const selectedStrategy = strategies.find(
          (s) => s.name === formData.trading_strategy,
        );
        if (selectedStrategy) {
      ...
      if (formData.trade_notes)
        formDataObj.append("trade_notes", formData.trade_notes);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-black mt-1 text-sm sm:text-base">
              ...
              {!backendConnected && (
                <span className="text-orange-600 ml-2">⚠️ Offline</span>
              )}
            </p>
          </div>
          {!showForm && (
            <button
              ...
              className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-[#3b82f6] text-white rounded-lg sm:rounded-xl font-semibold hover:bg-[#2563eb] transition-all duration-200 shadow-lg flex items-center justify-center gap-2 text-sm sm:text-base"
            >
              <svg
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3b82f6] text-black bg-white"
                        placeholder="AAPL"
                        required
                      />
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-end pt-4 border-t border-slate-200">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="w-full sm:w-auto px-6 py-2.5 bg-[#3b82f6] text-white text-sm font-medium rounded-lg hover:bg-[#2563eb] transition disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                      {submitting ? (
                        <>
        ...
        <div className="xl:col-span-2 bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg border border-white/20">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-black mt-1 text-xs sm:text-sm">
          ...
          <div className="mb-6">
            <div className="relative">
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 text-xs sm:text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3b82f6] focus:border-transparent bg-white text-black"
              />
              <svg
          ...
          {strategyTrades.length === 0 ? (
            <div className="text-center py-8 sm:py-12">
              ...
              <p className="text-black text-xs sm:text-sm">
                Add trades with strategies to see performance
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="block md:hidden space-y-4">
                {strategyTrades.map((strategy, index) => (
                  <div
                    key={index}
                    className="bg-gradient-to-r from-white to-slate-50 rounded-lg p-4 border border-slate-200 shadow-sm"
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-2 text-xs text-black mt-1">
                          <span className="flex items-center">
                            <div className="w-2 h-2 bg-[#3b82f6] rounded-full mr-1"></div>
                    ...
                    <button
                      ...
                      className="w-full bg-[#3b82f6] hover:bg-[#2563eb] text-white py-2 px-4 rounded-lg font-medium transition-colors text-sm"
                    >
                        ...
                        <TableRow key={index} hover sx={{ height: "72px" }}>
                          ...
                          <TableCell
                            ...
                            sx={{
                              ...
                              padding: "16px",
                              color:
                                strategy.total_pnl >= 0 ? "#10b981" : "#ef4444",
                            }}
                          >
                          ...
                          <TableCell align="center" sx={{ padding: "16px" }}>
                            <Tooltip title="View Trades">
                              <IconButton
                                size="small"
                                onClick={() =>
                                  handleViewStrategyTrades(strategy)
                                }
                                sx={{ color: "#3b82f6" }}
                              >
            ...
            <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 p-4 sm:p-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-blue-100 text-xs sm:text-sm mt-1">
                  {selectedStrategy.total_trades} trades • Win Rate:{" "}
            ...
            <div className="p-4 sm:p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-indigo-50 rounded-lg p-3 text-center">
                  ...
                  <p
                    className={`font-bold text-sm ${(selectedTrade.exit_price - selectedTrade.entry_price) * selectedTrade.quantity >= 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    {formatCurrency(
                      (selectedTrade.exit_price - selectedTrade.entry_price) *
                        selectedTrade.quantity,
                    )}
                  </p>
                </div>
              ...
              {/* Images */}
              {selectedTrade.image_urls &&
                selectedTrade.image_urls.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-slate-900 mb-3">
                      Images ({selectedTrade.image_urls.length})
                    </p>
