...
export default function MyJournal() {
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      ...
      if (
        ...
        setError(
          "Please fill in all required fields (Symbol, Entry Price, Exit Price, Quantity)",
        );
        setSubmitting(false);
        return;
      }
      ...
      if (formData.strategy_id) {
        ...
        const selectedStrategy = strategies.find(
          (s) => s.id === parseInt(formData.strategy_id),
        );
        if (selectedStrategy) {
  ...
  const confirmDeleteTrade = async () => {
    ...
    try {
      setDeleting(true);
      await apiCall(`/api/trade-logs/${tradeToDelete.id}`, {
        method: "DELETE",
      });
                    ...
                    >
                      ...
                      <TableCell sx={{ padding: "16px" }}>
                        <Chip
                          ...
                          color={
                            (trade.profit_loss || 0) >= 0 ? "success" : "error"
                          }
                          size="small"
                          sx={{ fontSize: "0.75rem" }}
                        />
              ...
              <div className="p-4 sm:p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p
                      className={`text-lg font-bold ${(selectedTrade.profit_loss || 0) >= 0 ? "text-green-600" : "text-red-600"}`}
                    >
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-slate-900">
                      {formatDate(selectedTrade.entry_date)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-600">Exit Date</p>
                    <p className="text-lg font-bold text-slate-900">
                      {formatDate(selectedTrade.exit_date)}
                    </p>
                  </div>
                </div>
                ...
                {selectedTrade.images && selectedTrade.images.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-slate-900 mb-3">
                      Images ({selectedTrade.images.length})
                    </p>
